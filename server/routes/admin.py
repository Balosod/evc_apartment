    ...
    for item in booked_property:
        property_obj = await Property.find_one(
            And(
                ...
                (Property.property_type == "EVC_Apartment"),
            )
        )
        if property_obj:
            try:
...
async def total_affiliate_sales() -> dict:
    ...
    for item in booked_property:
        property_obj = await Property.find_one(
            And(
                (Property.id == item.property_id),
                (Property.property_type == "EVCA_Affiliate"),
            )
        )
        if property_obj:
            try:
...
async def all_transaction() -> dict:
    ...
    all_transaction = await Transaction.find(
        Transaction.agent == "EVC_Apartment"
    ).to_list()
...
async def all_approved_property() -> dict:
    ...
    data = {"count": "", "data": ""}
    all_property = await Property.find(
        Property.approval_status == True, fetch_links=True
    ).to_list()
...
async def all_approved_property() -> dict:
    ...
    data = {"count": "", "data": ""}
    all_property = await Property.find(
        Property.approval_status == False, fetch_links=True
    ).to_list()
...
async def get_all_affiliate() -> dict:
    ...
    # Authorize.jwt_required()
    all_affiliate = []
    affiliate_obj = await User.find(
        And((User.is_affiliate == True), (User.active == True)), fetch_links=True
    ).to_list()
...
async def get_all_user() -> dict:
    ...
    # Authorize.jwt_required()
    all_user = []
    user_obj = await User.find(
        And((User.is_affiliate == False), (User.active == True)), fetch_links=True
    ).to_list()
