...
@router.post(
    "/signup", response_description="User added to the database", status_code=201
)
async def create_account(data: UserRegistrationSchema, response: Response) -> dict:
    ...
    if not match:
        response.status_code = 400
        return HTTPException(status_code=400, detail="Email is invalid")
    ...
    if phone_exists:
        response.status_code = 400
        return HTTPException(status_code=400, detail="Phone number already exists!")
    ...
    try:
        ...
        return HTTPException(status_code=400, detail="User not created")


@router.post("/auth/login", response_description="User login", status_code=200)
async def login_user(
    user: UserLogin, response: Response, Authorize: AuthJWT = Depends()
):
    user_acct = await User.find_one(User.email == user.email)
    try:
        if (
            user_acct
            and user_acct.active
            and pwd_context.verify(user.password, user_acct.password)
        ):
...
@router.post(
    ...
    status_code=200,
    response_description="Upload profile image",
)
async def upload_profile_image(
    data: ImageSchema, response: Response, Authorize: AuthJWT = Depends()
):
...
async def update_profile_data(
    data: ProfileDataSchema, response: Response, Authorize: AuthJWT = Depends()
):
