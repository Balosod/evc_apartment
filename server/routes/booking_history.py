...
async def get_ID(itemID):
    ...
    if propertyID:
        return {
            ...
            "agent": propertyID.property_type,
            "owner": propertyID.owner_id,
        }
    else:
        return None
...
async def book_a_property(
    ...
    if Id:
        ...
        total_statistic_data = await TotalStatistic.find_one(
            And(
                (TotalStatistic.date == today.strftime("%b-%d-%Y")),
                (TotalStatistic.agent == Id["agent"]),
            )
        )
        if total_statistic_data:
            ...
            new_statistic_data = TotalStatistic(
        ...
        if Id["agent"] == "EVCA_Affiliate":
            each_statistic_data = await EachStatistic.find_one(
                And(
                    (EachStatistic.date == today.strftime("%b-%d-%Y")),
                    (EachStatistic.owner_id == Id["owner"]),
                )
            )
            if each_statistic_data:
                ...
                each_statistic_data = EachStatistic(
                    date=today.strftime("%b-%d-%Y"),
                    reach=data.apply_discount,
                    owner_id=Id["owner"],
                )
        ...
        return HTTPException(status_code=400, detail="Property doesn't exit any more")
